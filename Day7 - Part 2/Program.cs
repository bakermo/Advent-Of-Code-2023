...
public class Hand : IComparable<Hand>
{
    ...
    private HandType RankHand()
    {
        ...
        if (uniqueCards.ContainsKey(CardRank.Joker))
        {
            int jokerCount = uniqueCards[CardRank.Joker];
            // basically, if they are all jokers, then its already 5 of a kind
            if (jokerCount < 5)
            {
                uniqueCards.Remove(CardRank.Joker);
                var highest = uniqueCards
                    .OrderByDescending(c => c.Value)
                    .FirstOrDefault();
                uniqueCards[highest.Key] += jokerCount;
            }  
        }
